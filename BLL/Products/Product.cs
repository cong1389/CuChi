...
namespace Cb.BLL.Product
{
    ...
    public class ProductBLL
    {
        ...
        public IList<PNK_Product> GetListRelate(int langId, string name, string newsCateId, string Id, int pageIndex, int pageSize, out int total)
        {
            ...
            string[] keyArr = param.Select(x => x.Value.ToString()).ToArray();
            string key = string.Format("Product_GetListRelate_{0}_{1}", WebUtils.CurrentUserIP, string.Join("_", keyArr));
            //Dictionary<string, object> dic = null;
            total = 0;
            ...
            //    dic = new Dictionary<string, object>();
            //    dic.Add("Dict_Product_GetRelate", lst);
            //    dic.Add("Dict_Product_GetRelate_Total", total);
            //    CacheHelper.Add(dic, key);
            //}
            //if (dic != null && dic.Count > 0)
            //{
            //    lst = dic["Dict_Product_GetRelate"] as IList<PNK_Product>;
            //    total = DBConvert.ParseInt(dic["Dict_Product_GetRelate_Total"]);
            //}
